...
public class Window3DController {
	...
	private void handleMouseClicked(final MouseEvent event) {
		...
		if (node instanceof Sphere) {
			...
			int idx = getPickedSphereIndex(picked);
			String name = normalizeName(cellNames[idx]);


			// check here if the visibility of this cell is above the threshold under which we won't 'select' this cell
		...
		// Structure
			...
			for (int i = 0; i < currentSceneElementMeshes.size(); i++) {
				...
				if (curr.equals(node)) {
					...
					boolean selectable = false;
					if (othersOpacityProperty.get() > VISIBILITY_CUTOFF) {
						// if others opacity is above the cutoff, it doesn't matter if this has a rule, it is selectable
						selectable = true;
					} else {
						// only act on a click of this node if there is an explicit rule which exempts it from the visibility cutoff
						for (Rule rule: rulesList) {
							if (rule.appliesToCellBody(name) || rule.appliesToStructureWithSceneName(name)) {
								selectable = true;
							}
							
							// only need one rule to apply for selectability so break here
							if (selectable) break;
						}
					}
